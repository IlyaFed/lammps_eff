...
class coordinate_visualisation(dash_object):
    def analyse(self, data, gen_info):
        ...
        if load_flag:
            return 0
        for item in self.index_list:
            self.data[item] = pd.Series(
                [np.zeros((4, n), dtype=float)]*self.data.shape[0])
    ...
    def load_step(self, Step):
        ...
        # highlight_ion
        our_param = parametrs[(parametrs['type'] == 1.0) & (
            parametrs['id'].isin(self.highlight_particles))]
        n = our_param.shape[0]
        ...
        grid_res = np.array(np.fromiter(
            grid, dtype=np.float64, count=self.grid_N ** 3))
        ...
        for i in range(self.grid_N):
            for j in range(self.grid_N):
                for k in range(self.grid_N):
                    grid[i][j][k] = grid_res[i * self.grid_N *
                                             self.grid_N + j * self.grid_N + k]
    ...
    def __get_scatter_trace(self):
        ...
        highlight_electrons = self.data.loc[self.current_index,
                                            'highlight_electrons']
        traces.append(go.Scatter3d(
    ...
    def __get_types_trace(self):
        ...
        # real name of every type ( 1 - e, 2 - H+)
        types = self.gen_info['particles_types']
        coord_type = self.data.loc[self.current_index,
                                   'particle_type']  # type of every partice
    ...
    def __get_surface_trace(self):
        ...
        vertices, simplices = measure.marching_cubes_classic(
            self.data.loc[self.current_index, 'surface'], iso_value)
        x, y, z = zip(*vertices)
    ...
    def __get_layout(self):
        if self.graph_type == 'surface':
            layout = go.Layout(
                title="Visualisation, isovalue from {:f} to {:f}".format(
                    self.isovalue_maxmin[1], self.isovalue_maxmin[0])
            )
        else:
            layout = go.Layout(
                scene=dict(
            ...
            html.Div(
                dcc.RadioItems(
                    ...
                    options=[{'label': i, 'value': i} for i in [
                        'scatter', 'surface', 'section x', 'section y', 'section z', 'types']],
                    value='types',
    ...
    def __init__(self, highlight_particles=[]):
        ...
        self.wall = [20., 20., 20.]
        self.index_list = ['coord_ions', 'coord_electrons', 'coord_wall',
                           'surface', 'highlight_ions', 'highlight_electrons']
